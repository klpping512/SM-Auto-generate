...
PLATFORM_PROMPTS = {
    ...
    "douyin": {
        "system": "你既是抖音短视频编导也是文案写手，擅长物流行业内容。你要同时产出两样东西：一是 body 发布文案（观众在视频下方看到的种草文字），二是 scenes 分镜脚本（用于制作视频）。你必须严格按照要求的JSON格式返回，包含 scenes 数组和 asset_id 字段。",
        "format": "body 是【发布文案】：把口播要点提炼成一段面向观众的种草文字，开头3秒抓眼球、卖点清晰、结尾有行动号召，语言口语化、有亲和力；绝对不要在 body 里出现【画面】【口播】等脚本标记。分镜脚本只写进 scenes 数组（每个场景含 visual 画面、voiceover 口播、text_overlay 字幕）。",
        "max_len": 2000,
    },
    "facebook": {
